...
public class DatabaseInitializer {
    ...
    private static final Shelter[] shelters = new Shelter[Shelter.ShelterCount];


    public static void populateAsync(final ShelterRoomDatabase db) {
        PopulateDbAsync task = new PopulateDbAsync(db);
        task.execute();
    }

    public static void populateSync(final ShelterRoomDatabase db) {
        populateWithShelterData(db);
    }

    private static Shelter addShelter(final ShelterRoomDatabase db, Shelter shelter) {
        db.shelterDao().insertAll(shelter);
        return shelter;
    }

    private static void populateWithShelterData(ShelterRoomDatabase db) {
        ...
        for (int i = 0; i < Shelter.ShelterCount; i++) {
            shelters[i] = new Shelter(shelterData[i][0], shelterData[i][1], shelterData[i][2], shelterData[i][3],
                                    shelterData[i][4], shelterData[i][5], shelterData[i][6], shelterData[i][7],
                                    shelterData[i][8]);
        }

        for (int i = 0; i < shelters.length; i++) {
            addShelter(db, shelters[i]);
        }
    }

    //private class that performs Async population of database
    private static class PopulateDbAsync extends AsyncTask<Void, Void, Void> {

        private final ShelterRoomDatabase mDb;

        PopulateDbAsync(ShelterRoomDatabase db) {
            mDb = db;
        }

        @Override
        protected Void doInBackground(final Void... params) {
            populateWithShelterData(mDb);
            return null;
        }
    }
}
